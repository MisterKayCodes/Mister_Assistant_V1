...
class SchedulerService:
    ...
    def schedule_task_reminders(self, task_id, user_id, end_time_str):
        ...
        self.scheduler.add_job(
            'services.scheduler_service:send_task_warning', # String reference (Senior Pro Tip)
            'date',
        ...
        # Job 2: Final Check
        self.scheduler.add_job(
            'services.scheduler_service:send_final_check', # String reference
            'date',
            run_date=end_time,
            args=[task_id, user_id],
            id=f"final_{task_id}",
...
# --- Global Job Functions (Rule: No 'self' to avoid pickling Bot) ---
...
async def send_task_warning(task_id, user_id):
    ...
    repo.update_user_state(str(user_id), state_context="WAITING_FOR_TASK_LOG")
